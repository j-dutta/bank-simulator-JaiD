...
#NEW FUNCTIONS
# Deposit and Withdraw Graph for the User to see
            ...
            # Edit email
                ...
                while True:
                    ...
                    if new_pin is None:
                        ...
                        eg.msgbox("Invalid PIN. Must be 4 digits.")
                        continue
...
def main_menu(account_info):
    while True:
        choice = eg.buttonbox("Main Menu", choices=["Deposit", "Withdraw", "Check Balance","Deposit and Withdrawal Graph", "Transaction History", "Exit" ]) # Main Menu Choices 
        if choice == "Deposit":
...
def show_transcations_graph(account_info):
    ...
    if not transactions: # If there is no transcations
        eg.msg("No Transactions to Display")
