...
def login_sys():
    while True:
        ...
        if login_options == 1:
            ...
            for account in account_data: #For account_info{} in account_data[]; the finds the account_info in account data
                if account['card number'] == cardnum_input: #Finding the card name in account_data[] and it will then match it and print out first name and last name
                    print(f"\nLogin Successful. Welcome {account['first name']} {account['last name']}!") #Print out the first and last name
        ...
        #Option 2 Pin Number
            ...
            if not (pinnum_input.isdigit() and len(pinnum_input) == 4): #Making sure it's an digit and making sure its len(4) long.
...
def account_creation():
    while True:
        ...
        if not first_name.isalpha() or not last_name.isalpha():
            print("\nInvalid. No spaces, No numbers or symbols allowed.")
            continue
...
def withdraw(account_info):
    while True:
        ...
        while True:
            ...
            if opt_reference == 1:
                user_reference = input("\nEnter Reference.\nEnter: ").lower()
                account_info['transactions'].append(f'Withdraw Amount ${wdr_amount:.2f} - {user_reference}') # Add with reference
                break
            elif opt_reference == 2:
                account_info['transactions'].append(f'Withdraw Amount ${wdr_amount:.2f}') # Add without reference
                break
            else:
                print('\nInvalid Option')
